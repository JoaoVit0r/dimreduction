...
/**
 * RadixSort implements a specialized version of the Radix Sort algorithm for sample sorting.
 * <p>
 * This class provides an implementation of the Radix Sort algorithm specifically designed 
 * for sorting samples based on feature values. It is primarily used in mean conditional 
 * entropy calculations, which are crucial for feature selection and network inference tasks.
 * <p>
 * The sorting process follows these steps:
 * <ol>
 *   <li>Create queues for each possible feature value</li>
 *   <li>Process features in reverse order (least significant to most significant)</li>
 *   <li>Distribute samples to queues based on feature values</li>
 *   <li>Restore samples from queues back to the original array</li>
 * </ol>
 * 
 * @see fs.MainCLI
 * @see utilities.FIFOQueue
 */
public class RadixSort {
    ...
    public static void radixSort(char[][] v, Vector I, int n) {
        ...
        queues = null;
        if (fs.MainCLI.isManualGCEnabled()) {
            System.gc();
        }
    }
