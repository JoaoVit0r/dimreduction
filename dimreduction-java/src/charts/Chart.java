...
public class Chart extends javax.swing.JFrame {
    ...
    public static void GraficoTorta() {
        ...
        pieDataset.setValue("JavaWorld", Integer.valueOf(75));
        pieDataset.setValue("Other", Integer.valueOf(25));
    ...
    public static void Distribution(AGN network) {
        ...
        xydatasets[0] = new XYSeriesCollection(serie);
        String title = "Distribuição Cumulativa dos graus dos vértices da rede - modelo " + network.getTopology();
    ...
    public static void Histogram(AGN network) {
        ...
        for (i = 0; i < maxdegree + 1; i++) {
            histogram.addValue((Number) (degreedistribution[i] / (float) network.getGenes().length), 0, i);
        }
                        ...
                        for (int t = 0; t < gene.getPredictorsties().length; t++){
                            for (int p = 0; p < gene.getPredictorsties()[t].size(); p++){
                                ...
                                if (predictor.getPathway()!=null){
                                    for (int pw = 0; pw < predictor.getPathway().size(); pw++){
                                        int pos = FindPathwayPosition(pathways, (String) predictor.getPathway().get(pw));
                                        if (pos != -1){
