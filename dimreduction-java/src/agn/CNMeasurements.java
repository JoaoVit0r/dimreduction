...
public class CNMeasurements {
    ...
    public static boolean hasVariation(char[][] M, boolean isperiodic) {
        ...
        for (int row = 0; row < M.length - 2 && !change; row++) {
    ...
    public static int[] Grade(int[] indegrees, int[] outdegrees, int node) {
        //grau do n�, considereando as posicoes, 0 = in-degree, 1 = out-degree.
    ...
    public static float[] ConfusionMatrix(AGN agn, AGN recovered, float threshold) {
        ...
        //6  == True Positives, just considering hubs
        //7  == False Positives, just considering hubs
        //8  == True Negatives, just considering hubs
        //9  == False Negatives, just considering hubs
        //10 == Ratio between true negatives and total negatives (TNR), just considering hubs
        //11 == Ratio between true positives and total positives (TPR), just considering hubs
        float[] CM = new float[12];
            ...
            for (int j = 0; j < agn.getNrgenes(); j++) {
                if (automat[ii][j] == 0) {
                    if (automat[ii][j] == recmat[ii][j]) {
                        CM[8]++;//true negative, just considering hubs
                    } else {
                        CM[7]++;//false positive, just considering hubs
                    }
                } else if (automat[ii][j] == 1) {
                    if (automat[ii][j] == recmat[ii][j]) {
                        CM[6]++;//true positive, just considering hubs
                    } else {
                        CM[9]++;//false negative, just considering hubs
                    }
                }
            }
        ...
        CM[10] = (CM[8] / (CM[8] + CM[7]));//TNR, just considering hubs
        CM[11] = (CM[6] / (CM[6] + CM[9]));//TPR, just considering hubs
        System.out.println("just considering hubs, result TNR = " + CM[10] + " , TPR = " + CM[11] + " and Similarity = " + Math.sqrt(CM[10] * CM[11]) + "\n\n");
        return (CM);
    }
