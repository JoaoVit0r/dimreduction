...
def log_to_csv(log_file_path, output_csv_path):
    ...
    pattern = r'monitoring_plots/\d+_\d+/(?P<environment>venv_\w+)/(?P<script>[^/]+)/distribution_(?P<distribution_type>[^/]+)/threads_(?P<thread_count>\d+)/.*\.txt:(?P<timestamp>\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}) Execution_(?P<execution_num>\d+)_(?P<event_type>Start|End) \w+_\d+( \(Duration: (?P<duration>(\d+h )?(\d+m )?\d+s)\))?( (?P<genes>\w+)genes)?'
    ...
    with open(log_file_path, 'r') as file:
        for line in file:
            ...
            if match:
                ...
                key = f"{environment}_{script}_{distribution_type}_{thread_count}_{execution_num}_{genes}"
                    ...
                    if duration_str:
                        ...
                        if duration_parts:
                            ...
                            if duration_parts.group(2):
                                ...
                                hours = 0
                            if duration_parts.group(4):
                                minutes = int(duration_parts.group(4))
                            else:
                                minutes = 0
                            seconds = int(duration_parts.group(5))
